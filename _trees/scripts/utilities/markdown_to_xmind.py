#!/usr/bin/env python3
"""
Markdown to XMind Converter (Enhanced)
...
def convert_tree_to_xmind_nodes(parent_element, tree_node, level=0):
    ...
    # Create XML node attributes
    node_attrs = {
        'CREATED': timestamp,
        'ID': tree_node['id'],
        'MODIFIED': timestamp,
        'TEXT': tree_node['name'],
        'MD_HASH': tree_node.get('hash', ''),
        'MD_LEVEL': str(tree_node.get('level', 0)),
        'MD_LINE': str(tree_node.get('md_line', 0))
    }
...
def generate_xmind_from_trees(trees, output_path, mapping_path):
    """Generate XMind XML from multiple tree structures."""
    # Create XML structure
    xmind_map = ET.Element('map', {
        'version': '1.0.1',
        'source': 'MARKDOWN',
        'timestamp': generate_timestamp()
    })
    ...
    p3.text = "Generated from markdown source files - do not edit directly."
    
    # Add special metadata attribute for tracking
    metadata = ET.SubElement(root_node, 'attribute', {
...
def process_path_files(path_dir, output_file, mapping_file):
    ...
    # Generate XMind file
    generate_xmind_from_trees(trees, output_file, mapping_path)
